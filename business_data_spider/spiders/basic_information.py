...
class BasicInformation(scrapy.Spider):
    ...
    def __init__(self, **kwargs):
        ...
        # 请求页面拼接的数据
        self.post_data = {
            "cata_id": str(settings['BASIC_INFO_ID']),  # 目录号
    ...
    def parse(self, response):
        # 获取数据
        businesses_info = json.loads(bytes.decode(response.body), object_pairs_hook=OrderedDict)

        # 判断是否数据为空
        if len(businesses_info['rows']) == 0:
            scrapy.Spider.close(BasicInformation, reason="All data has been collected.")

        # 解析数据
        for business in businesses_info['rows']:
            # 增量爬虫需要用到的字段
        ...
        # 最大上限页数
        max_page = businesses_info['total']
